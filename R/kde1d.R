...
#' @param fast logical, only relevant when \code{xmin} or \code{xmax} are finite; 
#' option for faster bandwidth selection (see \emph{Details}). 
#' 
...
#' @importFrom ks hpi
#' @export
kde1d <- function(data, xmin = -Inf, xmax = Inf, bw = NULL, mult = 1, fast = TRUE) {
    ...
    ## bandwidth selection
    n.subs <- ifelse(fast, min(500, length(data)), length(data))
    if (is.null(bw)) 
        bw <- NA
    if (is.na(bw))  
        bw <- bw_kde1d(data, xmin, xmax, n.subs)
